...
public class FeedbackController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if(flag!=null && flag.equals("userfeedback")){
			userfeedback(request, response);
		}
		else if(flag!=null && flag.equals("gfeedback")){
			gfeedback(request, response);}
		/*else{
		...
		response.sendRedirect("Company/Post_Feedback.jsp");
...
*/	}
	...
	void gfeedback(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		TenderDAO tdao=new TenderDAO();
		TenderVO tvo=new TenderVO();
		List ls=new ArrayList();
	...
	void userfeedback(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		TenderDAO tdao=new TenderDAO();
		TenderVO tvo=new TenderVO();
		List ls=new ArrayList();
		...
		System.out.println("load done");
		response.sendRedirect("User/Post_Feedback.jsp");
	...
	{
		System.out.println("inside insert method in feedback controller");
		...
		FeedbackDAO atdao=new FeedbackDAO();
		atdao.insert(cvo);
...
protected void searchg(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		FeedbackDAO aDAO = new FeedbackDAO();
			...
			HttpSession session=request.getSession();
			session.setAttribute("search1",ls);
			response.sendRedirect("Government_Employee/View_Feedback.jsp");
...
protected void searchu(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	FeedbackDAO aDAO = new FeedbackDAO();
		...
		HttpSession session=request.getSession();
		session.setAttribute("search1",ls);
		response.sendRedirect("User/View_Feedback.jsp");
