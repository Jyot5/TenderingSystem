...
public class CityDAO {
	...
	public List load(CityVO cityvo)
	{
	...
	public List search(CityVO catvo)
	{
	...
	public List edit(CityVO catvo) {
		...
		try {
			...
			session = sessionFactory.openSession();
			Transaction tr = session.beginTransaction();
			Query q = session.createQuery("from CityVO where city_ID = '"+catvo.getCity_ID()+"'");
			ls = q.list();
	...
	public Boolean delete(CityVO cvo){
		Session session = null;
		...
		Configuration().configure().buildSessionFactory();
		 ...
		 CityVO cvo1=(CityVO)session.get(CityVO.class, cvo.getCity_ID());
		 session.delete(cvo1);
